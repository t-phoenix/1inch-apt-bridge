...
library TakerTraitsLib {
    ...
    function createTakerTraits(
        ...
        uint takerTraits = 0;
        if (hasTarget) {
            takerTraits |= _ARGS_HAS_TARGET;
        }
        takerTraits |= (extensionLength& _ARGS_EXTENSION_LENGTH_MASK)  << _ARGS_EXTENSION_LENGTH_OFFSET;
        takerTraits |= (interactionLength  & _ARGS_INTERACTION_LENGTH_MASK )<< _ARGS_INTERACTION_LENGTH_OFFSET;
        // Set as maker mode
        takerTraits |= _MAKER_AMOUNT_FLAG;
        return TakerTraits.wrap(takerTraits);
    }
    /**
